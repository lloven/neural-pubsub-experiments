...
| H-RR-SATURATE | -- | Ablation | sat-5/10/15 x oracle/rr/market x 3 pipelines | 3x3x3x5 = 135 |
...
|----------|--------|---------------|---------------|-----------------|
| failure-{5,8,10}-12 | Information completeness | Kill 12 workers (25% capacity, VM2 only) at load L | L pps, kill all VM2 workers | 3×2 failure factorial varies load × kill ratio. 12-kill at low load (5 pps) is the sub-saturation baseline (48% util on survivors); at high load (10 pps) surviving 36 workers run at ~96% util, forcing the routing layer to make non-trivial decisions |
| failure-{5,8,10}-24 | Information completeness | Kill 24 workers (50% capacity, VM1+VM2) at load L | L pps, kill all edge workers | Severe capacity loss. Survivors span 72% util (5 pps) to 145% util (10 pps, over-saturation). Market should reject unaffordable pipelines via prices; rr-global queues unboundedly |
| sat-5 | Admission control | Sub-saturation baseline (5 pps, ~36% util) | 5 pps, no failure | Reference regime; all strategies should tie |
| sat-10 | Admission control | At-saturation (10 pps, ~72% util, below the knee) | 10 pps, no failure | System still sustains throughput, p99 begins to spread |
| sat-15 | Admission control | Above-saturation (15 pps, ~109% util, past the knee) | 15 pps, no failure | rr-global queues unboundedly; p99 blow-up (14.75s in calibration); market should reject via congestion pricing |
| heterogeneous | Price discovery | Edge VMs 2x slower, cloud 1.5x faster | `WORKER_PROCESSING_SPEED=2.0` on VM1/VM2; `=0.67` on VM3/VM4, 5 pps | rr-global splits load equally and bottlenecks on slow workers; market discovers fast workers via bid-scaled prices |
...
**Factorial design rationale (H-RR-RECOVER):** the 3×2 design (3 loads × 2 kill ratios) decomposes the failure response into separable effects: load main effect (does the market's price-signal advantage grow with arrival rate?), kill-ratio main effect (does it grow with capacity loss?), and load×kill interaction (do the two stresses compound?). Loads are picked so surviving workers span sub-saturation (5 pps, 12-kill = 48% util on the 36 survivors) to over-saturation (10 pps, 24-kill = 145% util on the 24 survivors). The factorial isolates the regime where information completeness via prices actually matters: surviving workers near or above saturation, where every wasted retry cascades into queueing.

**Saturation sweep rationale (H-RR-SATURATE):** calibrated against the 2026-04-18 saturation sweep (`results/calibration/SWEEP-RESULTS.md`). Two prior rate choices were wrong. First, 20/25/30 pps produced null results when the workload generator silently defaulted to 1.0 pps due to the L53 env-var bug. After the L53 fix, a deliberate sweep at 5/10/15/20/30/50 pps on oracle-global × cqi-chain × 48 workers found the knee at 10–15 pps: CR drops from 100% to 80.7%, p99 from 1992 ms to 14750 ms. Effective saturation is ~13.8 pps. Second theoretical estimate of ~200 pps was off by a factor of 14. The ablation now sweeps 5/10/15 pps (sub-saturation / at-knee / past-knee) so rr-global's unbounded queueing and market-quad's price-based rejection are both observable within the measurement window.
...
**Phase runner:** `scripts/run_ablation.py`. Standard CLI: `--configs failure-5-12,failure-8-12,failure-10-12,failure-5-24,failure-8-24,failure-10-24,sat-5,sat-10,sat-15,heterogeneous`, `--strategies`, `--pipelines`, `--seeds`, `--resume`. Run via `python -m scripts.run_ablation --topology distributed --resume` after the main campaign completes.
