...
class TestPhaseBMatrixSize:
    ...
    def test_full_matrix_is_expected_runs(self):
        """5gtn script: 5 configs x 2 transports x 5 seeds = 50 runs.

        The 5gtn shell script defaults to B1,B1eq,B2,B3,B4 (no B2flat).
        """
        ...
        # 5gtn defaults: 5 configs (excl B2flat) x 2 transports x 5 seeds
        _5gtn_configs = ["B1", "B1eq", "B2", "B3", "B4"]
        expected = expected_run_count("B", configs=_5gtn_configs)
        assert len(run_lines) == expected, (
            f"Expected {expected} runs, got {len(run_lines)}.\n"
            f"Output:\n{output[-2000:]}"
        )

    def test_single_config_is_expected_runs(self):
        """1 config x 2 transports x 5 seeds runs."""
        result = run_5gtn_script("phase-b", "--dry-run", "--configs", "B2")
        output = result.stdout + result.stderr
        # Count "Would run:" lines only
        run_lines = [l for l in output.splitlines() if "Would run:" in l]
        expected = expected_run_count("B", configs=["B2"])
        assert len(run_lines) == expected, (
            f"Expected {expected} runs for B2, got {len(run_lines)}.\n"
            f"Output:\n{output[-2000:]}"
        )
...
class TestDispatcherConfigsPassthrough:
    ...
    def test_configs_flag_forwarded_to_phase_b(self):
        ...
        only plan B1eq runs, not all configs."""
        result = run_root_script("phase-b", "--configs", "B1eq", "--dry-run")
        output = result.stdout + result.stderr
        assert result.returncode == 0, f"Script failed:\n{output}"
        expected = expected_run_count("B", configs=["B1eq"])
        assert f"{expected} runs planned" in output, (
            f"--configs B1eq should produce {expected} runs.\nOutput:\n{output[-2000:]}"
        )

    def test_configs_flag_forwarded_multiple(self):
        """--configs B1,B2 should produce expected runs."""
        result = run_root_script("phase-b", "--configs", "B1,B2", "--dry-run")
        output = result.stdout + result.stderr
        assert result.returncode == 0, f"Script failed:\n{output}"
        expected = expected_run_count("B", configs=["B1", "B2"])
        assert f"{expected} runs planned" in output, (
            f"--configs B1,B2 should produce {expected} runs.\nOutput:\n{output[-2000:]}"
        )
...
class TestDispatcherPhaseE:
    ...
    def test_phase_e_configs_forwarded(self):
        """--configs E7 should plan only E7 runs."""
        result = run_root_script("phase-e", "--configs", "E7", "--dry-run")
        output = result.stdout + result.stderr
        assert result.returncode == 0, f"Script failed:\n{output}"
        expected = expected_run_count("E", configs=["E7"])
        assert f"{expected} runs planned" in output, (
            f"--configs E7 should produce {expected} runs.\nOutput:\n{output[-2000:]}"
        )
